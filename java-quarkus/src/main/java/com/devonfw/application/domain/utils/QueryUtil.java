package com.devonfw.application.domain.utils;
...
/**
 * Class with utility methods for dealing with queries. Either extend this class
 * or use {@link QueryHelper#get()}.
 *
 * @since 3.0.0
 */
public class QueryUtil {
  ...
  public static final String QUERY_PROPERTY_TIMEOUT = "javax.persistence.query.timeout";
   ...
   */
  public void applyTimeout(JPAQuery<?> query, Number timeout) {
  ...
  /**
   * @param query          the {@link JPAQuery} to modify.
   * @param expression     the {@link StringExpression} to
   *                       {@link StringExpression#like(String) create the
   *                       LIKE-clause}
   *                       from.
   * @param pattern        the pattern for the LIKE-clause to create.
   * @param syntax         the {@link LikePatternSyntax} of the given
   *                       {@code pattern}.
   * @param ignoreCase     - {@code true} to ignore the case, {@code false}
   *                       otherwise (to search case-sensitive).
   * @param matchSubstring - {@code true} to match also if the given
   *                       {@code pattern} shall also match substrings on the
   ...
   * @param query          the {@link JPAQuery} to modify.
   * @param expression     the {@link StringExpression} to
   *                       {@link StringExpression#notLike(String) create the NOT
   *                       LIKE-clause} from.
   ...
   *                       {@code pattern} shall also match substrings on the
   *                       given {@link StringExpression}.
   * @param negate         - {@code true} for
   *                       {@link StringExpression#notLike(String) NOT LIKE},
   *                       {@code false} for
   ...
   * @param config     the {@link StringSearchConfigTo} to configure the search.
   *                   May be {@code null} for regular equals
   *                   search as default fallback.
   * @return the new {@link BooleanExpression} for the specified string comparison
   *         clause.
   */
   ...
   * @param syntax         the {@link LikePatternSyntax} of the given
   *                       {@code pattern}.
   * @param operator       the {@link StringSearchOperator} used to compare the
   *                       search string {@code value}.
   * @param ignoreCase     - {@code true} to ignore the case, {@code false}
   *                       otherwise (to search case-sensitive).
   * @param matchSubstring - {@code true} to match also if the given
   *                       {@code pattern} shall also match substrings on the
   ...
   *                   search.
   */
  public void whereString(JPAQuery<?> query, StringExpression expression, String value,
  ...
  /**
   * @param <T>        generic type of the {@link Collection} values for the
   *                   {@link SimpleExpression#in(Collection)
   *                   IN-clause}(s).
   * @param expression the {@link SimpleExpression} used to create the
   *                   {@link SimpleExpression#in(Collection)
   *                   IN-clause}(s).
   * @param inValues   the {@link Collection} of values for the
   *                   {@link SimpleExpression#in(Collection) IN-clause}(s).
   * @return the {@link BooleanExpression} for the
   *         {@link SimpleExpression#in(Collection) IN-clause}(s) oder
   *         {@code null} if {@code inValues} is {@code null} or
   ...
   *                   {@link SimpleExpression#in(Collection)
   *                   IN-clause}(s).
   * @param query      the {@link JPAQuery} where to
   *                   {@link JPAQuery#where(com.querydsl.core.types.Predicate)
   *                   add} the
   *                   {@link #newInClause(SimpleExpression, Collection)
  ...
  /**
   * Returns a {@link Page} of entities according to the supplied {@link Pageable}
   * and {@link JPAQuery}.
   *
   * @param <E>            generic type of the entity.
   * @param pageable       contains information about the requested page and
   *                       sorting.
   * @param query          is a query which is pre-configured with the desired
   *                       conditions for the search.
   * @param determineTotal - {@code true} to determine the
   *                       {@link Page#getTotalElements() total number of hits},
   *                       {@code false} otherwise.
   * @return a paginated list.
   */
  public <E> Page<E> findPaginatedGeneric(Pageable pageable, JPAQuery<E> query, boolean determineTotal) {
   ...
   * @param sort  the {@link Sort} to apply as ORDER BY to the given
   *              {@link JPAQuery}.
   */
  @SuppressWarnings("rawtypes")
  public void applySort(JPAQuery<?> query, Sort sort) {
