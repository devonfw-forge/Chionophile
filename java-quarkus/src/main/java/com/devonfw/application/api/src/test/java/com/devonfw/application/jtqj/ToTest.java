...
public class ToTest extends ModuleTest {
  ...
  static {
    // EXCLUSIONS.add(MyBusinessEto.class).ignoreEquals().ignoreProperties("foo",
    // "bar");
  }

  /**
   * Finds all {@link AbstractTo Transportobjekte} of this app via reflection and
   * tests all getters/setters as well as equals and hashCode.
   */
  @Test
  public void testTos() {
  ...
  private void testEqualsAndHashcode(Class<?> clazz, SoftAssertions assertion, ToExclusion exclusion) {
    ...
    EqualsVerifierApi<?> verifier = EqualsVerifier.forClass(clazz).withRedefinedSuperclass().usingGetClass()
      ...
      ReflectionUtils.doWithLocalFields(clazz, field -> {
        try {
          if (isCheckable(clazz, field, exclusion)) {
            ...
            if (getMethod == null) {
              assertion
                  .fail("Getter of field " + clazz.getSimpleName() + "." + field.getName() + " has not been found.");
            } else if (setMethod == null) {
              assertion
                  .fail("Setter of field " + clazz.getSimpleName() + "." + field.getName() + " has not been found.");
            } else {
              ...
              assertion.assertThat(value2)
                  .as("Value from getter of field " + clazz.getSimpleName() + "." + field.getName()).isEqualTo(value);
            }
          }
        } catch (Exception e) {
  ...
  public static class ToExclusion {
    ...
    /**
     * Ignores this transfer-object for testing of equals and hashCode with
     * {@link EqualsVerifier}.
     *
     * @return this
     */
